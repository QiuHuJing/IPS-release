...
public class Option {
    ...
    public static void init(boolean firstLoad) {
        ...
        // 中文默认：若语言未配置，使用 zh_cn
        if (!OPTIONS_DEFAULTS.containsKey(ParkourOption.LANG) || OPTIONS_DEFAULTS.get(ParkourOption.LANG).isEmpty()) {
            OPTIONS_DEFAULTS.put(ParkourOption.LANG, "zh_cn");
        }
    ...
    private static void initEnums() {
        ...
        try {
            ...
            SOUND_TYPE = Sound.valueOf("BLOCK_NOTE_BLOCK_PLING");
        ...
        try {
            ...
            // 1.21+ 兼容：许多旧粒子名称已更改
            IP.logging().error("Invalid particle type: %s — using END_ROD".formatted(value));
            try {
                PARTICLE_TYPE = Particle.valueOf("END_ROD");
            } catch (IllegalArgumentException ex2) {
                PARTICLE_TYPE = Particle.valueOf("END_ROD"); // 最后的回退
            }
        }
        ...
        // 1.21+ 使用无数据的粒子类型（避免 missing required data 错误）
        try {
            PARTICLE_DATA = new ParticleData<>(PARTICLE_TYPE, null, 10, 0, 0, 0, 0);
        } catch (Exception ex) {
            IP.logging().warn("Particle type %s requires data, using END_ROD instead".formatted(PARTICLE_TYPE));
            PARTICLE_TYPE = Particle.valueOf("END_ROD");
            PARTICLE_DATA = new ParticleData<>(PARTICLE_TYPE, null, 10, 0, 0, 0, 0);
        }
    }
