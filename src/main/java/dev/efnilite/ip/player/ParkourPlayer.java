...
public class ParkourPlayer extends ParkourUser {
    ...
    public ParkourPlayer(@NotNull Player player, @NotNull Session session, @Nullable PreviousData previousData) {
        ...
        // 中文迁移：旧版默认语言是 "en"，如果玩家数据中存的是旧默认值，自动切换为 zh_cn
        String configDefault = Option.OPTIONS_DEFAULTS.getOrDefault(ParkourOption.LANG, "zh_cn");
        if ("en".equals(this.locale) && "zh_cn".equals(configDefault)) {
            this.locale = "zh_cn";
            this._locale = "zh_cn";
        }
    ...
    public void save(boolean async) {
        ...
        if (async) {
            FoliaTask.create(IP.getPlugin()).async().execute(write).run();
        } else {
            write.run();
        }
    }
    ...
    public void setup(Location to) {
        ...
        if (Config.CONFIG.getBoolean("options.inventory-handling")) {
            FoliaTask.create(IP.getPlugin()).delay(5).execute(() -> {
