...
public class ParkourGenerator {
    ...
    public void startTick() {
        // Folia: 使用 GlobalRegionScheduler 仅做视觉更新（安全操作）
        // 方块操作由 Events.PlayerMoveEvent 触发（在区域线程上）
        task = FoliaTask.create(IP.getPlugin())
            .repeat(20) // 每秒1次，仅用于记分板/旁观者更新
            .execute(this::visualTick)
            .run();
    }

    /**
     * 视觉更新 tick（记分板、旁观者、时间）。
     * 仅做非方块操作，安全运行在 GlobalRegionScheduler 上。
     */
    private void visualTick() {
        ...
        getPlayers().forEach(other -> {
            ...
            if (other.player != null && other.player.isOnline()) {
    ...
    /**
     * 跑酷主 tick —— 方块检测和生成。
     * Folia: 由 PlayerMoveEvent 触发，在玩家所在区域线程执行。
     */
    public void tick() {
        if (stopped) {
            return;
        ...
        if (lastStandingPlayerLocation == null) {
            return;
        }
