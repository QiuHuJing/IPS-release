...
import dev.efnilite.ip.hook.DecentHoloHook;
import dev.efnilite.ip.hook.FancyHoloHook;
...
/**
 * Infinite Parkour 主类 — Folia 1.21.x 适配 & 汉化版
 *
 * @author Efnilite, Folia adaptation & CN localization by 叶小光
 * Copyright (c) 2020-2024
 */
public final class IP extends ViPlugin {

    public static final String NAME = "<#FF6464><bold>无尽跑酷 Infinite Parkour<reset>";
    ...
    // 重写 onDisable 以绕过 ViPlugin 中不兼容 Folia 的 cancelTasks() 调用
    @Override
    ...
    public void enable() {
        ...
        // hook with hologram plugins & papi
        ...
        if (getServer().getPluginManager().isPluginEnabled("DecentHolograms")) {
            logging.info("Registered DecentHolograms hook");
            DecentHoloHook.init();
        }

        if (getServer().getPluginManager().isPluginEnabled("FancyHolograms")) {
            logging.info("Registered FancyHolograms hook");
            FancyHoloHook.init();
        }
